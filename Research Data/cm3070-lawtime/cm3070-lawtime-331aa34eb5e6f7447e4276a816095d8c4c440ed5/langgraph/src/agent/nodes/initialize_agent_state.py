...
async def initialize_agent_state(
    ...
    # Use Azure OpenAI API key (fallback to DASHSCOPE_API_KEY for backward compatibility)
    azure_openai_api_key = os.getenv("AZURE_OPENAI_API_KEY") or os.getenv("DASHSCOPE_API_KEY")
    if not azure_openai_api_key:
        raise ValueError(
            "AZURE_OPENAI_API_KEY or DASHSCOPE_API_KEY not found in environment variables - required for AI processing"
        )
    # Store as dashscope_api_key in state for backward compatibility with existing code

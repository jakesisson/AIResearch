...
async def classify_document_type(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        # Create LLM instance with Azure OpenAI
        ...
        if not azure_openai_api_instance and azure_openai_endpoint:
            # Extract instance name from endpoint URL (e.g., https://instance.openai.azure.com/)
            try:
        ...
        chat_llm = ChatOpenAI(
            ...
            api_key=state.get("dashscope_api_key"),  # Using dashscope_api_key field for backward compatibility
            temperature=0,
        )
        logger.info("ChatOpenAI (Azure OpenAI) LLM instance created successfully")
        ...
        logger.debug("Sending prompt to Azure OpenAI for document classification")

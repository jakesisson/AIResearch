...
class OpenAIProvider(ILLMProvider):
        ...
        if settings.azure_openai_endpoint and settings.azure_openai_api_key:
            ...
            if not settings.azure_openai_deployment:
                raise ValueError(
                    ...
                    "Please set AZURE_OPENAI_DEPLOYMENT in your .env file."
                )
            ...
            logger.info(f"Initialized Azure OpenAI provider with deployment: {settings.azure_openai_deployment}")
        else:
            # Use standard OpenAI
            if not settings.openai_api_key:
                raise ValueError(
                    "OPENAI_API_KEY is not configured. "
                    "Please set OPENAI_API_KEY in your .env file (or configure Azure OpenAI)."
                )
    ...
    async def get_model_name(self) -> str:
        ...
        Returns:
            Model name (e.g., "gpt-4-turbo-preview") or Azure deployment name
        """

#!/usr/bin/env python3
"""
Consistency: run cost-perf N times with distinct inputs (cold each time), compute
mean and stddev of duration; assert variance is bounded (e.g. p95 < 2 * p50).
Writes prior/researched metrics to .cost-perf-consistency.json for comparison.
"""
import json
import sys
import statistics
...
def test_clone(name: str, cwd: Path, variant: str) -> tuple[bool, dict]:
    ...
    elif mean_d > 0 and p95 > 2 * p50:
        print(f"  WARN: p95 {p95:.0f} > 2*p50 {2*p50:.0f}", file=sys.stderr)
    else:

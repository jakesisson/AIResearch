...
export default function EnhancedDashboard() {
  ...
  // حساب الإحصائيات من البيانات الحقيقية مع التحقق من صحة البيانات
  const opportunitiesArray = Array.isArray(opportunities) ? opportunities : [];
  const totalValue = opportunitiesArray.reduce((sum: number, opp: any) => sum + (opp.value || 0), 0) || 0;
  const activeOpportunities = opportunitiesArray.filter((opp: any) => opp.stage !== 'closed').length || 0;
    ...
    <Layout>
      ...
      <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-b border-slate-700/50">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="text-center">
            <h1 className="text-4xl font-bold text-white mb-4">مرحباً بك في سيادة AI</h1>
            <p className="text-xl text-slate-300 mb-6">نظام ذكي متكامل لإدارة الأعمال والتواصل مع العملاء</p>

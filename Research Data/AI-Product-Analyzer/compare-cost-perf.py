#!/usr/bin/env python3
"""
Runs cost-perf test on both prior and researched AI-Product-Analyzer clones, then writes a comparison report.
Loads master.env from AIResearch so Azure/LLM vars are available.

Usage (from Research Data/AI-Product-Analyzer):
  python compare-cost-perf.py

Requires: both clones present, venv with dependencies activated (or PYTHONPATH set), Azure env in master.env.
"""
import os
import sys
import json
...
PRIOR_DIR = ROOT / "AI-Product-Analyzer-cfe471701dd29b6c354874ad9015bf8957dc69b5"
RESEARCHED_DIR = ROOT / "AI-Product-Analyzer-6779ffda3a8a21dec8afb5b57bfee345a8a19798"
...
def run_test(cwd: Path, variant: str):
    ...
    # Prefer shared .venv in this folder if present, else current interpreter
...
def main():
    ...
    if not PRIOR_DIR.is_dir() or not RESEARCHED_DIR.is_dir():
        print("Prior or researched clone not found. Expected:", file=sys.stderr)
        print(f"  {PRIOR_DIR}", file=sys.stderr)
        print(f"  {RESEARCHED_DIR}", file=sys.stderr)
        sys.exit(1)
    ...
    report = {
        ...
        "delta": {
            "durationMs": researched_ms - prior_ms,

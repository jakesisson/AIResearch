...
class MessagePublic(MessageBase):
    id: uuid.UUID
    created_at: datetime
    token_usage: dict[str, int] | None = None  # Token usage info if available
    
    @classmethod
    def from_message(cls, message: "Message") -> "MessagePublic":
        """Create MessagePublic from Message, extracting token_usage from message_data."""
        token_usage = None
        if message.message_data and isinstance(message.message_data, dict):
            token_usage = message.message_data.get("token_usage")
...
class ConversationPublic(ConversationBase):
    ...
    def from_conversation(cls, conversation: "Conversation") -> "ConversationPublic":
        """Create ConversationPublic from Conversation, converting messages."""
        return cls(
            id=conversation.id,
            title=conversation.title,
            created_at=conversation.created_at,
            user_id=conversation.user_id,
            messages=[MessagePublic.from_message(msg) for msg in conversation.messages]
        )

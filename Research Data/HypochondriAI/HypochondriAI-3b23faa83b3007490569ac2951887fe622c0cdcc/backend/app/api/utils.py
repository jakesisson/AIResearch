...
async def get_and_save_ai_response(
    ...
    Returns:
        tuple[Message, dict]: The saved AI response message object and token usage info.
    """
    try:
        ...
        # Check if response has usage information (from Langchain callbacks or response metadata)
        ...
        if ai_response_metadata:
            ai_response_metadata["token_usage"] = token_usage
        else:
            ai_response_metadata = {"token_usage": token_usage}

#!/usr/bin/env python3
"""
Runs cost-perf on both prior and researched sys-scan-graph clones, then writes a comparison report.

Usage (from Research Data/sys-scan-graph):
...
PRIOR_DIR = ROOT / "sys-scan-graph-d0e0ea8a9d030175fb196f8f4f30ecf378bd0f10"
RESEARCHED_DIR = ROOT / "sys-scan-graph-b9a4506472e8dd660ce3d5d2e2b4f2d076a4d16f"
# When COST_PERF_STRESS=1, run_cost_perf in each clone uses cost_perf_fixture_report_stress.json


def get_python(_clone_dir: Path):
    # Prefer shared .venv so both prior and researched use the same deps
    shared_venv = ROOT / ".venv" / "bin" / "python"
    if shared_venv.exists():
        return str(shared_venv)
    shared_venv = ROOT / ".venv" / "Scripts" / "python.exe"
    if shared_venv.exists():
        return str(shared_venv)
    # Fallback: clone-specific agent/venv
    venv_py = _clone_dir / "agent" / "venv" / "bin" / "python"
    if venv_py.exists():
        return str(venv_py)
    venv_py = _clone_dir / "agent" / "venv" / "Scripts" / "python.exe"
    if venv_py.exists():
        return str(venv_py)
...
def main():
    if not PRIOR_DIR.is_dir() or not RESEARCHED_DIR.is_dir():
        ...
        sys.exit(1)
    if os.environ.get("COST_PERF_STRESS") == "1":
        print("COST_PERF_STRESS=1: using stress fixture (cost_perf_fixture_report_stress.json)\n")

...
if config.AZURE_OPENAI_API_KEY and config.AZURE_OPENAI_ENDPOINT:
    # For Azure OpenAI, use base_url with deployment
    # Format: https://{instance}.openai.azure.com/openai/deployments/{deployment}
    deployment = config.AZURE_OPENAI_API_DEPLOYMENT or config.MODEL_ID
    base_url = f"{config.AZURE_OPENAI_ENDPOINT.rstrip('/')}/openai/deployments/{deployment}"
    open_ai_client = OpenAI(
        api_key=config.AZURE_OPENAI_API_KEY,
        base_url=base_url,
        default_headers={"api-key": config.AZURE_OPENAI_API_KEY},
    )
    # Store api_version for use in API calls
    open_ai_client._api_version = config.AZURE_OPENAI_API_VERSION  # type: ignore
else:
    # Fallback to standard OpenAI
    if not config.OPENAI_API_KEY:
        raise ValueError("Either OPENAI_API_KEY or AZURE_OPENAI_API_KEY and AZURE_OPENAI_ENDPOINT must be provided")

...
from src.utils.openrouter_llm import create_llm, create_openrouter_llm, get_model_name
...
def create_byok_workflow(
    ...
    # Create LLMs with user's key and settings (create_llm auto-selects Azure OpenAI or OpenRouter)
    annotation_llm = create_llm(
...
async def lifespan(app: FastAPI):
    ...
    if llm_provider == "openrouter":
        ...
        # Create LLMs for each agent (create_llm auto-selects Azure OpenAI or OpenRouter)
        annotation_llm = create_llm(
            model=annotation_model,

...
class OpenAIModelComponent(LCModelComponent):
    ...
    inputs = [
        *LCModelComponent._base_inputs,
        ...
        StrInput(
            ...
            advanced=True,
            info="Azure OpenAI endpoint URL. If set, will use Azure OpenAI instead of standard OpenAI.",
        ),
        StrInput(
        ...
        StrInput(
            ...
            advanced=True,
            info="Azure OpenAI deployment name. If not set, will use model_name.",
        ),
        SecretStrInput(
            name="api_key",
    ...
    def build_model(self) -> LanguageModel:  # type: ignore[type-var]
        ...
        azure_api_key = SecretStr(self.api_key).get_secret_value() if self.api_key else None
        
        if azure_endpoint and azure_api_key:
            # Use Azure OpenAI
            azure_deployment = getattr(self, 'azure_deployment', None) or self.model_name
            azure_api_version = getattr(self, 'azure_api_version', None) or "2025-01-01-preview"
            ...
            logger.info(f"Using Azure OpenAI - Deployment: {azure_deployment}, Model: {self.model_name}")

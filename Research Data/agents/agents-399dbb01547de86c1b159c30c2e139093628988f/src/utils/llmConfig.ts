...
export const llmConfigs: Record<string, t.LLMConfig | undefined> = {
  ...
  [Providers.AZURE]: {
    ...
    // Extract instance name from endpoint if not provided directly
    azureOpenAIApiInstanceName: process.env.AZURE_OPENAI_API_INSTANCE || 
      (process.env.AZURE_OPENAI_ENDPOINT 
        ? process.env.AZURE_OPENAI_ENDPOINT.replace('https://', '').replace('.openai.azure.com', '').replace('/', '')
        : undefined),
    azureOpenAIApiDeploymentName: process.env.AZURE_OPENAI_API_DEPLOYMENT || process.env.MODEL_ID || 'gpt-4.1',
    azureOpenAIApiVersion: process.env.AZURE_OPENAI_API_VERSION || '2025-01-01-preview',
    model: process.env.AZURE_MODEL_NAME || process.env.MODEL_ID || 'gpt-4.1',
  },
  [Providers.OLLAMA]: {

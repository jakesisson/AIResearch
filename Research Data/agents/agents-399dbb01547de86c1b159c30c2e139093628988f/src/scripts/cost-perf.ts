/**
 * Cost & performance test for the agents LangGraph workflow (researched commit).
 * Tracks tokens (cost) and latency (performance). Loads master.env from AIResearch if present.
 * Set LANGFUSE_SECRET_KEY (e.g. in master.env) to send traces to Langfuse.
 */
import { config } from 'dotenv';
import * as path from 'path';
import * as fs from 'fs';

// Load .env then master.env before any other modules that read process.env (ESM hoists imports, so we dynamic-import those below)
config();
...
async function main() {
  ...
  if (process.env.LANGFUSE_SECRET_KEY) {
    console.log('Traces sent to Langfuse (tag: cost-perf).');
  }
}

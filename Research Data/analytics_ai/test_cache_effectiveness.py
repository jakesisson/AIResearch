...
analytics_ai has no application-level cache, so run2 will also use tokens; we still record prior/researched for comparison.
...
def test_clone(name: str, cwd: Path, variant: str) -> tuple[bool, dict]:
    ...
    passed = True  # both runs succeeded

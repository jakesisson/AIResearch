...
from openai import AzureOpenAI # Azure OpenAI API のインポート
...
def create_analysis_plan_node(state: MyState) -> MyState:
    ...
    try:
        ...
        # 関数定義（Azure OpenAI function calling形式）
        functions = [
            {
                "name": "create_plan",
        ...
        try:
            ...
            if message.tool_calls and len(message.tool_calls) > 0:
                ...
                try:
                    ...
                    logging.info("Azure OpenAIがテキスト形式で有効なプランを返しました。これを採用します。")
                    plan_steps = parsed_plan_from_text
                except (json.JSONDecodeError, ValueError) as e_text_parse:
                    logging.error(f"Azure OpenAIのテキストレスポンスのパースに失敗: {e_text_parse}")
            ...
            error_message = str(e_api) if hasattr(e_api, '__str__') else "Unknown error"
            return {**state, "analysis_plan": None, "current_plan_step_index": None, "user_clarification": None,
                    "condition": "plan_generation_failed", "error": f"分析計画の作成中にモデル呼び出しでエラーが発生しました。管理者にご連絡ください。(詳細: {error_message})"}
        except (ValueError, AttributeError, json.JSONDecodeError) as e_parse:
            logging.error(f"Azure OpenAIレスポンスの処理または検証中にエラーが発生しました: {e_parse}", exc_info=True)
            return {**state, "analysis_plan": None, "current_plan_step_index": None, "user_clarification": None,

...
class AigieConfig:
    ...
    # Azure OpenAI integration settings (replaces Gemini)
    enable_gemini_analysis: bool = True  # Kept for backward compatibility, now uses Azure OpenAI
    gemini_project_id: Optional[str] = None  # Kept for backward compatibility, ignored
    gemini_location: str = "us-central1"  # Kept for backward compatibility, ignored
    gemini_api_key: Optional[str] = None  # Kept for backward compatibility, can be used as fallback
    ...
    def from_environment(cls) -> 'AigieConfig':
        ...
        elif os.getenv("GEMINI_API_KEY") or os.getenv("AIGIE_GEMINI_API_KEY"):
            # Fallback to old Gemini key name for backward compatibility
            config.azure_openai_api_key = os.getenv("GEMINI_API_KEY") or os.getenv("AIGIE_GEMINI_API_KEY")
        
        if os.getenv("AZURE_OPENAI_ENDPOINT"):
            config.azure_openai_endpoint = os.getenv("AZURE_OPENAI_ENDPOINT")
        
        if os.getenv("AZURE_OPENAI_API_VERSION"):
            config.azure_openai_api_version = os.getenv("AZURE_OPENAI_API_VERSION", "2025-01-01-preview")
        
        if os.getenv("AZURE_OPENAI_API_DEPLOYMENT") or os.getenv("MODEL_ID"):
            config.azure_openai_deployment = os.getenv("AZURE_OPENAI_API_DEPLOYMENT") or os.getenv("MODEL_ID", "gpt-4.1")
        
        if os.getenv("MODEL_ID"):
            config.model_id = os.getenv("MODEL_ID", "gpt-4.1")
        
        # Legacy Gemini settings (kept for backward compatibility, but ignored)

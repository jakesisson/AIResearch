...
Please complete the task now with this enhanced context:"""
    ...
    def enable_gemini_analysis(self, project_id: Optional[str] = None, location: str = "us-central1"):
        """Enable Azure OpenAI-powered error analysis (GeminiAnalyzer name kept for backward compatibility)."""
        try:
            self.gemini_analyzer = GeminiAnalyzer(project_id, location)  # Uses Azure OpenAI now
            if self.gemini_analyzer.is_available():
                self.intelligent_retry = IntelligentRetry(self.gemini_analyzer)
                logging.info("Azure OpenAI-powered error analysis enabled")
            else:
                logging.warning("Azure OpenAI not available - check API key and endpoint")
        except Exception as e:
            logging.error(f"Failed to enable Azure OpenAI analysis: {e}")
    
    def get_gemini_status(self) -> Dict[str, Any]:
        """Get Azure OpenAI integration status (GeminiAnalyzer name kept for backward compatibility)."""
        if self.gemini_analyzer:
            return self.gemini_analyzer.get_status()
        return {"enabled": False, "reason": "Azure OpenAI not initialized"}

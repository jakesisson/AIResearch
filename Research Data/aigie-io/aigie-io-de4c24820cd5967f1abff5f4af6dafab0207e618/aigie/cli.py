...
def test_gemini_connection() -> int:
    ...
    try:
        ...
        # Try to create Azure OpenAI analyzer (GeminiAnalyzer class name kept for backward compatibility)
        ...
        if not api_key or not endpoint:
            print("❌ Azure OpenAI credentials not found. Set AZURE_OPENAI_API_KEY and AZURE_OPENAI_ENDPOINT environment variables")
            return 1
        
        analyzer = GeminiAnalyzer()  # Uses Azure OpenAI now
        
        if analyzer.is_available():
            print("✅ Azure OpenAI connection successful!")
            print(f"  Endpoint: {analyzer.endpoint}")
            print(f"  Deployment: {analyzer.deployment}")
            print(f"  Model: {analyzer.model_id}")
        else:
            print("❌ Azure OpenAI connection failed")
            print("Please check:")
            print("1. AZURE_OPENAI_API_KEY is correct")
            print("2. AZURE_OPENAI_ENDPOINT is correct")
            print("3. AZURE_OPENAI_API_VERSION is set (defaults to 2025-01-01-preview)")
